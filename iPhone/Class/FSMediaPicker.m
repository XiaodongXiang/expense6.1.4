...
- (void)delegatePerformWillPresentImagePicker:(LCImagePickerViewController *)imagePicker;
...
- (void)imagePickerController:(LCImagePickerViewController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info
{
...
- (void)navigationController:(UINavigationController *)navigationController willShowViewController:(UIViewController *)viewController animated:(BOOL)animated
{
    if ([viewController isKindOfClass:NSClassFromString(@"PLUIImageViewController")] && self.editMode && [navigationController.viewControllers count] == 3) {
        ...
        position=(screenHeight-SCREENWIDTH)/2;
...
- (void)showAlertController:(UIView *)view
{
    ...
    switch (self.mediaType) {
        case FSMediaTypePhoto:
        {
            [alertController addAction:[UIAlertAction actionWithTitle:NSLocalizedString(@"VC_Photo from Camera", nil) style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
                [self takePhotoFromCamera];
            }]];
            [alertController addAction:[UIAlertAction actionWithTitle:NSLocalizedString(@"VC_Photo from Library", nil) style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
                [self takePhotoFromPhotoLibrary];
            }]];
            break;
        }
    ...
    [alertController addAction:[UIAlertAction actionWithTitle:NSLocalizedString(@"VC_Cancel", nil) style:UIAlertActionStyleCancel handler:^(UIAlertAction *action) {
        [self delegatePerformCancel];
    }]];
...
- (void)takePhotoFromCamera
{
    if ([LCImagePickerViewController isSourceTypeAvailable:UIImagePickerControllerSourceTypeCamera]) {
        LCImagePickerViewController *imagePicker = [LCImagePickerViewController new];
...
- (void)takePhotoFromPhotoLibrary
{
    if ([LCImagePickerViewController isSourceTypeAvailable:UIImagePickerControllerSourceTypePhotoLibrary]) {
        LCImagePickerViewController *imagePicker = [LCImagePickerViewController new];
