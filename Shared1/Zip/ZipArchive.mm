...
//	tm_zip filetime;
	...
	else
	{
		...
		crcValue = (uLong)(crc32( crcValue, (const Bytef*)[data bytes], (int)[data length] ));
...
-(BOOL) UnzipOpenFile:(NSString*) zipFile
{
	...
	if( _unzFile )
	{
		...
		{
            //需要将NSString创建与打印分开来写
            NSString *logStr = [NSString stringWithFormat:@"%lu entries in the zip file",globalInfo.number_entry];
            NSLog(@"%@",logStr);
        }
	}
