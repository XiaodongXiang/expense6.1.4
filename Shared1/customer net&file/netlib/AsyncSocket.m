...
//如果是在IOS设备上跑的话，就需要额外的添加一个CFNetwork的框架
#if TARGET_OS_IPHONE
...
//定义同步的当前状态，是准备好了buffer,已经连接上了，已经打开了读进程，已经打开了写进程。。。。
...
- (unsigned)readLengthForTerm
{
	...
	unsigned long result = [term length];
	
	// Shortcut when term is a single byte
	if(result == 1) return (unsigned)result;
	...
	if(maxLength > 0)
		return (int)MIN(result, (maxLength - bytesDone));
	else
		return (int)result;
}
...
{
	if(maxLength > 0)
		return (unsigned)MIN(READALL_CHUNKSIZE, (maxLength - bytesDone));
	else
...
- (void)runLoopAddSource:(CFRunLoopSourceRef)source
{
	unsigned long i, count = [theRunLoopModes count];
	for(i = 0; i < count; i++)
	{
...
- (BOOL)acceptOnPort:(UInt16)port error:(NSError **)errPtr
{
	...
	return [self acceptOnAddress:nil port:50001 error:errPtr];
}
...
//	cliaddr.sin_port = htons(50505); //BTW：1024到5000间的端口是系统用于自动分配的，小心了
//	cliaddr.sin_addr.s_addr = htonl(INADDR_ANY);
//	
//	bind(CFSocketGetNative(theSocket), &cliaddr, sizeof(cliaddr));
...
//	sin.sin_port = htons(50505); // port we want to connect on
...
//	CFSocketError e = CFSocketConnectToAddress(socket,address,timeout); 
	CFSocketSetDefaultNameRegistryPortNumber(50001);
	...
	if(theCurrentRead != nil && theReadStream != NULL)
	{
		...
		while(!done && !socketError && !maxoutError && [self hasBytesAvailable])
		{
			...
			if(theCurrentRead->readAllAvailableData == YES)
			{
				...
				unsigned long buffInc = READALL_CHUNKSIZE - ([theCurrentRead->buffer length] - theCurrentRead->bytesDone);
			...
			if(theCurrentRead->term != nil)
			{
				...
				if(([partialReadBuffer length] > 0) || !(theFlags & kEnablePreBuffering))
				{
					...
					unsigned long bufInc = maxToRead - ([theCurrentRead->buffer length] - theCurrentRead->bytesDone);
				...
				else
				{
					...
					unsigned long buffInc = maxToRead - ([theCurrentRead->buffer length] - theCurrentRead->bytesDone);
					[theCurrentRead->buffer increaseLengthBy:buffInc];
			...
			if(theCurrentRead->readAllAvailableData != YES)
			{
				if(theCurrentRead->term != nil)
				{
					...
					else
					{
						...
						unsigned long termlen = [theCurrentRead->term length];
