...
#if TARGET_OS_IPHONE
#define POST_CHUNKSIZE  (1024 * 1024 * 1024)
#else
#define POST_CHUNKSIZE  (1024 * 1024 * 1024)
#endif
	...
	else
	{
		...
		else
		{
			...
			if([ranges count] == 1)
			{
				...
				unsigned int bytesToRead = (unsigned int)(range.length < READ_CHUNKSIZE ? range.length : READ_CHUNKSIZE);
...
- (void)onSocket:(AsyncSocket *)sock didReadData:(NSData*)data withTag:(long)tag
{
	if(tag == HTTP_REQUEST_HEADER)
	{
		...
		else
		{
			...
			if(expectsUpload)
			{
				...
				uint bytesToRead = (unsigned int)(requestContentLength < POST_CHUNKSIZE ? requestContentLength : POST_CHUNKSIZE);
	...
	else
	{
		...
		if(requestContentLengthReceived < requestContentLength)
		{
			...
			uint bytesToRead = (unsigned int)(bytesLeft < POST_CHUNKSIZE ? bytesLeft : POST_CHUNKSIZE);
...
- (void)onSocket:(AsyncSocket *)sock didWriteDataWithTag:(long)tag
{
	...
	else if(tag == HTTP_PARTIAL_RANGE_RESPONSE_BODY)
	{
		...
		if(bytesLeft > 0)
		{
			unsigned int bytesToRead = (unsigned int)(bytesLeft < READ_CHUNKSIZE ? bytesLeft : READ_CHUNKSIZE);
